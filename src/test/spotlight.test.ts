...
/** Whoever the ad is for. Most of these tests only care that the slot is taken. */
function advertiser(name: string) {
  return {
    name,
    normalizedWebsite: `${name.toLowerCase()}.com`,
    displayWebsite: `https://${name.toLowerCase()}.com`,
...
describe("spotlight reservation", () => {
  it("holds the slot so a second buyer cannot reach checkout", async () => {
    await reserveSpotlight({ slot: "premium", advertiser: advertiser("l1"), listingId: "l1", priceCents: PREMIUM_CENTS });

    await expect(
      reserveSpotlight({ slot: "premium", advertiser: advertiser("l2"), listingId: "l2", priceCents: PREMIUM_CENTS }),
  ...
  it("keeps the two slots independent", async () => {
    await reserveSpotlight({ slot: "premium", advertiser: advertiser("l1"), listingId: "l1", priceCents: PREMIUM_CENTS });
    await expect(
      reserveSpotlight({ slot: "standard", advertiser: advertiser("l2"), listingId: "l2", priceCents: 900 }),
  ...
  it("frees the slot again when checkout could not be created", async () => {
    const first = await reserveSpotlight({
      slot: "premium",
      advertiser: advertiser("l1"), listingId: "l1",
...
describe("spotlight activation", () => {
  ...
  it("never lets two payments own the same slot at the same time", async () => {
    ...
    await fakeDb.collection(COLLECTIONS.spotlightBookings).doc("stale").set({
      advertiser: advertiser("l2"), listingId: "l2",
      slot: "premium",
      priceCents: PREMIUM_CENTS,
      startsAt: null,
      endsAt: null,
