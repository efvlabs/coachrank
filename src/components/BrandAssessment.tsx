...
export function BrandAssessment() {
  ...
  useEffect(()=>{
    ...
    async function load(){
      try{
        const response=await fetch("/api/assessment",{cache:"no-store",signal:controller.signal});const data=await response.json();
  ...
  async function save(action:string,payload:unknown){
    ...
    try{
      const response=await fetch("/api/assessment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({revision:order.revision,action,payload})});const data=await response.json();
  ...
  async function olderReports(){if(!runs.length||busy)return;setBusy(true);setError("");try{const response=await fetch(`/api/assessment/history?before=${runs[0].index}`,{cache:"no-store"});const data=await response.json();if(!response.ok)throw new Error(data.error);setRuns(old=>mergeReports(old,data.reports));}catch(error){setError(error instanceof Error?error.message:"Could not load earlier reports.");}finally{setBusy(false);}}
  ...
  if(!order||order.status!=="paid")return <div className="tool-state"><p className="journal-label">Brand Clarity Assessment</p><h1>{order?.status==="pending"?"Confirming your payment.":order?.status==="reversed"?"This purchase has been reversed.":"Let’s find your assessment."}</h1><p>{order?.status==="pending"?"Your assessment will open after Dodo confirms payment. This can take a moment. There is no need to pay again.":order?.status==="reversed"?"Access closes when a payment is refunded or reversed. Contact us if you need help.":"Already purchased? Open your saved private access link, or return in the browser you used at checkout."}</p>{error?<p className="tool-error" role="alert">{error}</p>:null}<div className="tool-state-actions"><button className="tool-button" onClick={()=>window.location.reload()}>Check access again</button><Link href="/tools/brand-clarity" className="tool-text-link">Explore the assessment</Link></div><p className="tool-tax-note">Need help? Email <a href="mailto:contact@coachrank.lol">contact@coachrank.lol</a> with your Dodo receipt. Never send card details.</p></div>;
