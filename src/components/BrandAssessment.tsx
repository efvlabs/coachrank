...
export function BrandAssessment({ preview = false, orderId }: { preview?: boolean; orderId?: string }) {
  const query = preview ? "preview=true" : orderId ? `order=${encodeURIComponent(orderId)}` : "";
  const endpoint = `/api/assessment${query ? `?${query}` : ""}`;
  ...
  useEffect(()=>{
    ...
    async function load(){
      try{
        const response=await fetch(endpoint,{cache:"no-store",signal:controller.signal});let data=await response.json();
        if(!preview && response.ok && ["pending","failed"].includes(data.order?.status) && checks%5===0){const verified=await fetch(`/api/assessment/verify${query ? `?${query}` : ""}`,{method:"POST",signal:controller.signal});if(verified.ok)data=await verified.json();}
  ...
  async function olderReports(){if(!runs.length||busy)return;setBusy(true);setError("");try{const response=await fetch(`/api/assessment/history?before=${runs[0].index}${query ? `&${query}` : ""}`,{cache:"no-store"});const data=await response.json();if(!response.ok)throw new Error(data.error);setRuns(old=>mergeReports(old,data.reports));}catch(error){setError(error instanceof Error?error.message:"Could not load earlier reports.");}finally{setBusy(false);}}
  ...
  if(!order||order.status!=="paid")return <div className="tool-state"><p className="journal-label">Brand Clarity Assessment</p><h1>{preview ? "Open your admin preview." : order?.status==="pending"?"Confirming your payment.":order?.status==="reversed"?"This purchase has been reversed.":order?.status==="failed"?"Your payment was not completed.":"Let’s find your assessment."}</h1><p>{preview ? "Sign in to CoachRank Studio and choose Open admin preview. Your saved preview reports will be there." : order?.status==="pending"?"Your assessment will open after Dodo confirms payment. This can take a moment. There is no need to pay again.":order?.status==="reversed"?"Access closes when a payment is refunded or reversed. Contact us if you need help.":order?.status==="failed"?"You can return to checkout from the product page. If you see a charge on your receipt, check access again or contact us before retrying payment.":"Sign in with the account you used to buy this assessment. Your progress and reports are saved in My tools. Earlier purchases can still use their private access link."}</p>{error?<p className="tool-error" role="alert">{error}</p>:null}<div className="tool-state-actions"><button className="tool-button" onClick={()=>window.location.reload()}>Check access again</button><Link href={preview ? "/admin/tools" : customerSignInUrl(orderId ? `/tools/brand-clarity/assessment?order=${orderId}` : "/my-tools")} className="tool-text-link">{preview ? "Open CoachRank Studio" : "Sign in to find your assessment"}</Link>{order?.status==="failed" && <Link href="/tools/brand-clarity#get-assessment" className="tool-text-link">Return to checkout</Link>}</div><p className="tool-tax-note">Need help? Email <a href="mailto:contact@coachrank.lol">contact@coachrank.lol</a> with your Dodo receipt. Never send card details.</p></div>;
  ...
  return <div className="assessment-workspace">
    ...
    {finished&&selected?<>
      ...
      <BrandReport orderId={order.id} preview={preview} run={selected.index} compareRun={baseline?.index??-1} answers={selected.answers} previous={baseline?.answers} completedAtMs={selected.completedAtMs} previousAtMs={baseline?.completedAtMs}/>
