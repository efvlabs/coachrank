...
export default function PrivacyPage() {
  ...
  return (
    ...
    >
      ...
      <p>When you sign in with Google or an email link, Firebase Authentication verifies your identity. We associate your Firebase user ID and email with your assessment purchases. Your display name, when provided by Google, is used to identify your session. We request no access to your Google mail, contacts or files. Signing in does not subscribe you to marketing emails.</p>
      <p>The <code>cr_customer</code> cookie keeps your account signed in for up to 14 days. It is HTTP-only, Secure in production and SameSite Lax. Sign in again to restore access after it expires or on another device. Your browser temporarily stores the email used to request a sign-in link so the link can be completed on the same device; it is removed after successful sign-in. Firebase sends the authentication email and applies its own abuse controls.</p>
      <p>The legacy <code>cr_brand_access</code> cookie lasts up to one year. For earlier purchases that are not yet connected to an account, the private link remains an access credential: anyone holding it can view the answers and reports or connect that purchase to their account. Keep it private. Once connected, sign-in to the owning account is required. Admin previews use a separate <code>cr_brand_preview</code> cookie and require administrator sign-in. Signing out of a customer account removes its session and assessment access cookie without deleting saved reports.</p>
      <p>We retain completed assessments to provide continued report access, or until you request deletion, subject to payment-record retention requirements. Authorized administrators can see purchase status and optional feedback. Contact contact@coachrank.lol to request account or report deletion or help with access. We verify ownership before acting on those requests.</p>
