...
export default async function AdminToolsPage() {
  ...
  const metrics = [
    ...
    { label: "Completed reports", value: paid.filter(order => order.completed.length).length, detail: "Buyers who finished their first report" },
    { label: "Buyer feedback", value: paid.filter(order => order.feedback).length, detail: "Preview runs are excluded" },
  ];
  return <div>
    <div className="admin-page-heading"><div><p className="journal-label">Tools &amp; feedback</p><h1>Turn insight into action.</h1><p>Brand Clarity Assessment · $9 USD · one useful next step.</p></div><AssessmentPreviewButton /></div>
